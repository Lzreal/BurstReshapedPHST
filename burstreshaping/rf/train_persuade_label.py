...
def train_DF(conf):
    ...
    proxy_dataset = RFDataset(**proxy_data_conf)
    logger.info("Start training....")

    hs_data_conf = conf["data_conf"]["train_conf"]["hs_conf"]
    hs_data_conf.update(const_conf)
    hs_dataset = RFDataset(**hs_data_conf)
    
    model.fit(
        proxy_dataset.train_data,
        proxy_dataset.train_label,
        batch_size=model_conf["batch_size"],
        epochs=model_conf["nb_epoch"],
        verbose=0,
        callbacks=[loguru_callback],
        validation_data = (proxy_dataset.val_data,proxy_dataset.val_label),
    )

    for _ in range(model_conf["pseudo_steps"]):
        
        predictions = model.predict(hs_dataset.train_data)
        
        confidence_index = np.where(predictions.max(axis=1) > confidence_threshold)
        
        train_data = np.concatenate(
            [proxy_dataset.train_data, hs_dataset.train_data[confidence_index]]
        )
        ...
        logger.info(
            f"pseudo_steps: {_+1}/{(model_conf['pseudo_steps'])}, train_data shape is {train_data.shape}."
        )
        ...
        train_label,
        batch_size=model_conf["batch_size"],
        epochs=model_conf["epoch_per_pseudo"],
        verbose=0,
        callbacks=[loguru_callback],
        validation_data = (proxy_dataset.val_data,proxy_dataset.val_label),
        )   
